...
pub struct TauriState {
    pub pool: SqlitePool,
    pub devices: HashMap<String, Status>,
}

impl TauriState {
    pub fn new(pool: SqlitePool) -> Self {
        // lấy danh sách device đã lưu trong db

        // lấy danh sách device đang kết nối với máy tính

        // kiểm tra, so sánh trạng thái của các thiết bị
...
pub async fn start_server<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        let listener = tokio::net::TcpListener::bind(format!("{_adr}:{_port}"))
            .await
            .unwrap();
